            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('type') ? 'is-invalid' : '' }}" name="type" id="type">
                    ...
                    @foreach(App\Models\Transaksi::TYPE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('type', $transaksi->type) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
                @if($errors->has('type'))
                    <div class="invalid-feedback">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('tikets') ? 'is-invalid' : '' }}" name="tikets[]" id="tikets" multiple>
                    @foreach($tikets as $id => $tiket)
                        <option value="{{ $id }}" {{ (in_array($id, old('tikets', [])) || $transaksi->tikets->contains($id)) ? 'selected' : '' }}>{{ $tiket }}</option>
                    @endforeach
                </select>
